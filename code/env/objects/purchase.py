...
class Purchase(object):
    def __init__(self, config, supply_type):
        self.key = config['node_code']
        self.type = supply_type
        self.material = config['material_code']

        self.period = config['period']
        self.lower_storage = config['min']
        self.upper_storage = config['max']
    ...
    def get_state(self):
        state = {
            'key': self.key,
            'lower_storage': self.lower_storage,
            'upper_storage': self.upper_storage,
            'demand': self.demand
        }
        return state
    ...
    def get_reward(self, step_cnt):
        ...
        for idx, p in enumerate(self.period):
            if step_cnt % p == 0:
                quantity = self.lower_storage[idx]
                if self.key == 'SINOPEC':
                    material_idx = idx
        ...
        for idx, material in enumerate(self.material):
            short_storage = self.lower_storage[idx] - self.storage[idx]
            loss_storage = self.storage[idx] - self.upper_storage[idx]

            sa_reward += max(0, short_storage) * 10
            sl_reward += max(0, loss_storage) * 20

        return [-t_reward, -sa_reward, -sl_reward, 0, 0, 0]
